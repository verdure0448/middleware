 ...
 */
public class BlcokBatchPollingHealthcheckHandler extends AbstractTransactionTimeoutFunctionHandler {
	...
	public BlcokBatchPollingHealthcheckHandler(String name, long timeout, MitsubishiQSeriesApi api, List<RequestReadObj> readObjList,
	...
	public void transactionProcess(IContext inboundCtx, OutboundContext outboundCtx) throws UrlParseException {
		...
		try {
			...
			try {
				...
				rowData = api.multipleRead(readObjList);
				jsonData = hexToJson(rowData, readObjList, _schema);
			...
			// PLC 쓰기 기능
			if (writeObjList != null) {
				try {
					JSONParser parser = new JSONParser();
					JSONObject jsonObject = (JSONObject) parser.parse(jsonData);
					jsonObject = (JSONObject) jsonObject.get("Pc");
					String pcCheck = (String) jsonObject.get("Check");
					
					if(pcCheck.equals("0001")) {
						writeObjList.get(0).setData("0000");
						api.multipleWrite(writeObjList);						
					}else {
						writeObjList.get(0).setData("0001");
						api.multipleWrite(writeObjList);						
					}
