...
public class SocketChannelConnection implements IConnection{
	...
	public SocketChannelConnection(SelectionKey key, int ioReadBuffer, BlockingQueue queue){
	...
	public synchronized int gatlingRead() throws IOException {
		int ioValue = channel.read(readBuffer);
		if(ioValue>0){
			readBuffer.flip();
			queue.blockingenqueue(new QueueEntry(this, ByteBuffer.wrap(readBuffer.array())));
			readBuffer.clear();
		}
		return ioValue;
	}
	...
	public class QueueEntry{
		...
		private QueueEntry(IConnection con, ByteBuffer packet){
			this.con = con;
			this.packet = packet;
		}
