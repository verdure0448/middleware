...
public abstract class AbstractSingleAdapterInstanceProcessor implements IAdapterProcessor, IContextCallback, IAdapterProcessorEventListener{
	...
	public AbstractSingleAdapterInstanceProcessor(ICommonService service, IAdapterContext ctx){
		...
		this.addEventListener(this); //프로세서의 처리중 IOException이 발생하면 인스턴스를 자동으로 정지시킨다. 
	}
...
//	}
	...
	private void responseInnerTransfer(IContext inboundCtx, OutboundContext outboundCtx) throws Exception{
		try{
			transfer(outboundCtx);
...
//		String devPoolId = this.instanceInfo.getDevPoolId();
	...
	@Override
	public void onEvent(IAdapterProcessorEvent e) {
		int stateType = e.getAdapterProcessEventStateType();
		switch(stateType){
		case IAdapterProcessorEvent.STATE_ERROR:
			aic.asyncProcess(IAdapterInstanceEvent.STOP_EVENT);
			log.warn("연결이 끊어져서 인스턴스를 정지 시킴.");
			break;
		default:
			return;
		}
