 ...
 *
 */
public class UserPutHandler extends AbstractFunctionHandler {

	private IProfileManager pm;

	public UserPutHandler(IProfileManager pm) {
		super("put");
		this.pm = pm;
	}
	...
	public void process(IContext inboundCtx, OutboundContext outboundCtx) throws Exception {
		...
		// 유저구분
		mUser.userType((String) inputJson.get(WebSocketAdapterConst.USER_TYPE));
		// 유저명
		...
		// ContextHandlerApplicationException(1031,
		// CommonException.TYPE_WARNNING, "유저ID가 도메인에 존재합니다.");
		if (domainObj != null)
		...
		// CommonException.TYPE_WARNNING, "유저 정보가 중복입니다.");
		if (userObj != null)
		...
		if (userPoolObj == null)
			throw getCommonService().getExceptionfactory().createAppException(this.getClass().getName() + ":020",
					new String[]{(String) inputJson.get(WebSocketAdapterConst.UPID), uid});
		...
		try {
			mDomainObj.domainId(uid).domainNm((String) inputJson.get(WebSocketAdapterConst.USER_NAME))
					.domainType(WebSocketAdapterConst.UID).remark("").insert();
		...
		try {
			...
			// throw new ContextHandlerApplicationException(302,
			// CommonException.TYPE_ERROR, "데이터 등록에 실패 했습니다.", ex);
