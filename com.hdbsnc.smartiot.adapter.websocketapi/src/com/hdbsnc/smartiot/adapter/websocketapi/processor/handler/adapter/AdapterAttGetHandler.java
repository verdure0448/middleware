...
/**
 * adt/att/get 아답터 속성 정보를 조회한다.
 * 
 * @author KANG
 *
 */
public class AdapterAttGetHandler extends AbstractFunctionHandler {

	private IAdapterManager am;

	public AdapterAttGetHandler(IAdapterManager am) {
		super("get");
		this.am = am;
	}
	...
	public void process(IContext inboundCtx, OutboundContext outboundCtx) throws Exception {
		...
		// if (iAdt == manifest) throw new ContextHandlerApplicationException(2001, CommonException.TYPE_ERROR, "아답터 매니페스트 정보가 존재하지 않습니다.");
		if (manifest == null) throw getCommonService().getExceptionfactory().createAppException(this.getClass().getName() + ":010", new String[]{aid});
		
		if (manifest.getAttributes().size() == 0) throw getCommonService().getExceptionfactory().createAppException(this.getClass().getName() + ":015", new String[]{aid});
		
		JSONArray jsonArray = new JSONArray();
		JSONObject jsonAdapter = null;

		for (String att : manifest.getAttributes()) {
			jsonAdapter = new JSONObject();
			jsonAdapter.put(WebSocketAdapterConst.AID, aid);
			jsonAdapter.put(WebSocketAdapterConst.ADT_ATTRIBUTION, att);
